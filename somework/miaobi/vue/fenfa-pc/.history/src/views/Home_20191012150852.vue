    ...
    <div class="home">
                ...
                <div class="zanwu1" :class="{bacn: acceptedTasks.length>0}">
                    <div v-if="acceptedTasks.length>0" ref="mySwiper">
                        <div class="swiper-slide" v-for="item in acceptedTasks" :key="item.title">
                            ...
                            <!-- 句子对任务 -->
                            <router-link :to="`/juzi/${item.id}`"
                                         v-if="item.type===missionType.sentence">
                                <div class="btn start">开始任务</div>
                            </router-link>

                            <router-link :to="`/gaixie/${item.id}`"
                                         v-if="item.type===missionType.rewrite">
                                <div class="btn start">开始任务</div>
                            </router-link>

                            <router-link :to="`/tongyi/${item.id}`"
                                         v-if="item.type===missionType.synonym && item.status!==7">
                                <div class="btn start">开始任务</div>
                            </router-link>

                            <router-link :to="`/pingjia/${item.id}`"
                                         v-if="(item.type===missionType.scoring)">
                                <div class="btn start">开始任务</div>
                            </router-link>
                            <!-- 没有审核任务了 -->
        ...
        <el-dialog title="请选择任务类型"
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="applyTask" round>确认</el-button>
            </div>
        </el-dialog>
    </div>
...
<script>
    ...
    import {Button, Dialog, Form, FormItem, Select, Option, Progress} from "element-ui";
        ...
        methods: {
            ...
            applyTask() {
                this.applyForTask({
                    ...
                    "taskType": this.form.taskTypes[0].key
                }).then(() => {
...
<style lang="scss">
    ...
    .el-button--primary {
        background: linear-gradient(0deg, rgba(255, 155, 2, 1) 0%, rgba(255, 200, 1, 1) 88%);
        border: #ff9b02;
    }
