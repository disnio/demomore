<template>
  <el-row>
    <el-col :span="22" :offset="1">
      <div class="topbar">
        ...
        <el-select
          ...
          class="topbaroption fl"
        >
          <el-option v-for="(item, k) in filterStates" :key="k" :label="item" :value="k"></el-option>
        </el-select>
    ...
    <el-col :span="22" :offset="1">
      <el-table :data="missions" border stripe style="width: 100%" class="user-table">
        ...
        <el-table-column prop="submitDt" label="提交时间" width="100">
          <template slot-scope="scope">
            <p>{{ scope.row.submitDt[0] }}</p>
            <p>{{ scope.row.submitDt[1] }}</p>
          </template>
        </el-table-column>
        <el-table-column prop="firstAuditDt" label="一审完成时间" width="100">
          <template slot-scope="scope">
            <p>{{ scope.row.firstAuditDt[0] }}</p>
            <p>{{ scope.row.firstAuditDt[1] }}</p>
          </template>
        </el-table-column>
        <el-table-column prop="auditDt" label="二审完成时间" width="100">
          <template slot-scope="scope">
            <p>{{ scope.row.auditDt[0]}}</p>
            <p>{{ scope.row.auditDt[1] }}</p>
          </template>
        </el-table-column>
        <!-- <el-table-column prop="submitDt" label="审核时间" width="100">
                    <template slot-scope="scope">
                        <p>{{ scope.row.submitDt[0] }}</p>
                        <p>{{ scope.row.submitDt[1] }}</p>
                    </template>
        ...
        <el-table-column prop="firstAuditorName" label="一审审核人" width="100"></el-table-column>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            ...
            <el-button
              size="mini"
              type="danger"
              @click="closeMission(scope.row)"
              v-if="canClose(scope.row.taskStatus)"
            >关闭任务</el-button>
            <el-button
              size="mini"
              type="info"
              @click="disWriter(scope.row)"
              v-if="canDis(scope.row.taskStatus)"
            >调整写手</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 查看拒稿原因弹框 -->
    <el-dialog title="查看拒稿原因" @close="closeRefuse" :visible.sync="dialogVisible" width="30%" center>
      <p class="p-text">
        <span>任务名称：</span>
        {{refuseReason.name}}
      </p>
    ...
    <!--分配其他写手-->
    <el-dialog title="分配其他写手" :visible.sync="disWriterVisible" width="28%" size="medium" center>
...
export default {
  ...
  methods: {
    ...
    querySearch(v, cb) {
      ...
      if (v) {
        let rs = _.filter(arr, writer => {
          return (
            pt.test(writer.name.toLowerCase()) ||
            pt.test(writer.namePy.toLowerCase())
          );
        });
        cb(rs);
      }
    ...
    // 分配其他写手
    okDisWriter() {
      this.DistributionWriter({ tId: this.disTId, writerId: this.writerId })
        .then(res => {
    ...
    canDis(status) {
      ...
      return canDisStatus.indexOf(status) > -1;
    },
    canClose(status) {
      return status !== "已关闭";
    },
