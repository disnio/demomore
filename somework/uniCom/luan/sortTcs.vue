<!--
 * @Author: fengzp
 * @Date: 2021-06-01 15:37:59
 * @LastEditors: fengzp
 * @LastEditTime: 2021-06-17 19:19:04
  ...
  methods: {
    ...
    createCols() {
      ...
      if (this.columns) {
        ...
        if (
          this.tableCol.noWidth != '*' &&
          !this.tableCol.hasOwnProperty('noWidth')
        ) {
      ...
      if (
        ...
        !!this.tableMethod.lastOpts &&
        this.tableMethod.lastOpts.length > 0
      ) {
        if (
    ...
    mtInputChange(val, row, item, act) {
      ...
      if (index !== -1) {
        ...
        this.$emit('line-changeInput', this.list[index]);
        this.$emit('changeInput', this.list);
      }
    },
    // 输入框 blur 事件
